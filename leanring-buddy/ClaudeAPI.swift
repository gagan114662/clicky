...
/// How ClaudeAPI authenticates requests.
/// - proxyURL: routes through the Cloudflare Worker, which holds the API key.
/// - claudeCodeOAuth: calls api.anthropic.com directly using the OAuth bearer
///   token that Claude Code stores in the macOS Keychain after `claude login`.
enum ClaudeAuthMode {
    case proxyURL(String)
    case claudeCodeOAuth

    fileprivate var endpointURL: URL {
        switch self {
        case .proxyURL(let urlString):
    ...
    fileprivate var tlsWarmupURL: URL {
        switch self {
        case .proxyURL(let urlString):
            // Warm up the proxy host root
...
class ClaudeAPI: AnthropicChatClient {
    ...
    init(authMode: ClaudeAuthMode, model: String = "claude-sonnet-4-6") {
    ...
    /// Builds the base URLRequest with auth headers set for the current auth mode.
    /// For OAuth mode, the token is read fresh from the Keychain each call so we
    /// always use the token Claude Code has most recently refreshed.
    private func makeAPIRequest() throws -> URLRequest {
        var request = URLRequest(url: authMode.endpointURL)
        ...
        switch authMode {
        case .proxyURL:
            // The Cloudflare Worker injects the Anthropic API key server-side.
            break
        case .claudeCodeOAuth:
            let bearerToken = try ClaudeCodeOAuthProvider.readAccessToken()
